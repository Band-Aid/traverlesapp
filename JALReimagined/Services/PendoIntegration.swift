...
enum PendoIntegration {
    ...
    private static let fallbackDeepLinkScheme = "pendo-e0bd942a"
    private static var deepLinkScheme: String {
        guard let urlTypes = Bundle.main.object(forInfoDictionaryKey: "CFBundleURLTypes") as? [[String: Any]] else {
            return fallbackDeepLinkScheme
        }

        if let pendoScheme = urlTypes
            .compactMap({ $0["CFBundleURLSchemes"] as? [String] })
            .flatMap({ $0 })
            .first(where: { $0.hasPrefix("pendo-") }) {
            return pendoScheme
        }
    ...
    static func handleOpenURL(_ url: URL) -> Bool {
        guard url.scheme == deepLinkScheme else { return false }
